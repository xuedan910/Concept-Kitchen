	...
	foreach ( $args['fields'] as $key => $val ) {
		...
		switch ( $key ) {
			...
			case 's':
				...
				$out .= sprintf( '<input type="text" name="s" value="%1$s" class="%2$s form-control" placeholder="type and hit enter" id="%3$s" />',
			...
			default:
				...
				$out .= sprintf( '<select name="%1$s" class="%2$s" id="%3$s" style="width:184px;height:30px;border: solid 1px #ccc;border-radius:4px;padding-left:10px;" />',
					$tax->query_var,
					esc_attr( $val['class'] ),
					esc_attr( $val['id'] )
				);
